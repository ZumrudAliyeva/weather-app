...
import { apiKey, base } from "../../config/config";

export const getCityData = createAsyncThunk("city", async (obj) => {
    try {
        const request = await axios.get(`${base}/data/2.5/weather?q=${obj.city}&units=${obj.unit}&APPID=${apiKey}`);
...
export const get5DaysForecast = createAsyncThunk("5days", async (obj) => {
    const request = await axios.get(`${base}/data/2.5/forecast?lat=${obj.lat}&lon=${obj.lon}&units=${obj.unit}&APPID=${apiKey}`);
    const response = await request.data;
    return response;
})
